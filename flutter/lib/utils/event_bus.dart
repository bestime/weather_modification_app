class EventBus {
  ...
  Map create (String name, Function handle, bool isOnce, bool onlyOne) {
    item = {
      'id': ++_eventId,
      'name': name,
      'isOnce': isOnce,
  ...
  Map on (String name, Function handle) {
    return create(name, handle, false, false);
  }

  Map once (String name, Function handle) {
    return create(name, handle, true, false);
  }

  Map one (String name, Function handle) {
    return create(name, handle, false, true);
  }

  Map oneOnce (String name, Function handle) {
  ...
  bool emit (String name, [Map data = const {}]) {
...
Map bus1 = EventBus.on('永久事件', (Map data, Map busItem) {
  print('Bus执行 =>【$busItem】$data');
});

EventBus.once('一次性事件', (Map data, Map busItem) {
  print('Bus执行 =>【$busItem】$data');
});

EventBus.one('唯一事件', (Map data, Map busItem) {
  print('Bus执行 =>【$busItem】$data');
});

EventBus.oneOnce('唯一一次性事件', (Map data, Map busItem){
  print('Bus执行 =>【$busItem】$data');
});
